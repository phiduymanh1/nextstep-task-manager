...
public class AuthController extends BaseController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest req
        ...
        try {
            ...
            return ResponseEntity.ok(
                    success(MessageConst.REFRESH_SUCCESS, authResponse)
            );
        } catch (AuthenticationException ex) {
            return ResponseEntity.status(401).body(
                    error(MessageConst.INVALID_CREDENTIALS)
            );
        }
    }
    ...
    public ResponseEntity<?> refresh(@CookieValue(value = "refreshToken", required = false) String refreshToken) {
        if (refreshToken == null || !jwtUtil.isRefreshTokenValid(refreshToken)) {
            return ResponseEntity.status(401)
                    .body(error(MessageConst.REFRESH_TOKEN_INVALID));
        }
    ...
    public ResponseEntity<?> register(@Valid @RequestBody RegisterRequest req){
        ...
        return ResponseEntity.ok(
                success(MessageConst.REGISTER_SUCCESS, null)
        );
    }
    ...
    public ResponseEntity<?> forgotPassword(@RequestBody ForgotPasswordRequest request){
        ...
        return ResponseEntity.ok(
                success(MessageConst.SEND_LINK_FORGOT_PASSWORD, null)
        );
    }
