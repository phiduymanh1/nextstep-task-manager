...
import org.example.nextstepbackend.dto.request.CardMemberResponse;
...
public class CardService {
  ...
  private CardDetailResponse buildResponse(
    ...
    List<CardMemberResponse> memberResponses =
        members.stream()
            .map(
                u ->
                    new CardMemberResponse(
                        u.getId(), u.getUser().getFullName(), u.getUser().getAvatarUrl()))
            .toList();
